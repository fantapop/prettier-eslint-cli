...
function formatFilesFromArgv(_ref) {
  ...
  var eslintLoadPath = eslintPath || "eslint";
  var ConfigFile = require(`${eslintLoadPath}/lib/config/config-file`);
  var Linter = require(`${eslintLoadPath}/lib/linter`);
  var Config = require(`${eslintLoadPath}/lib/config`);

  if (eslintConfigPath) {
    var configContext = new Config({}, new Linter());
    prettierESLintOptions.eslintConfig = ConfigFile.load(eslintConfigPath, configContext);
  }
